...
anamoly_dir = "/users/chbandi/traces/cpc/anamoly/"
normal_dir = "/users/chbandi/traces/cpc/normal/"
cpc_dir = "/users/chbandi/traces/cpc/"
...
def dump_omni_results():
    finalstats()
    filepath = "/users/chbandi/traces/cpc/results/omni_results.json"
    with open(filepath, 'w') as file:
        json_string = json.dumps(omni_results, default=lambda o: o.__dict__, indent=4)
        file.write(json_string)

    filepath = "/users/chbandi/traces/cpc/results/finalstats.json"
    with open(filepath, 'w') as file:
        json_string = json.dumps(final_stats, default=lambda o: o.__dict__, indent=4)
        file.write(json_string)

    filepath = "/users/chbandi/traces/cpc/results/totalDuration.json"
    with open(filepath, 'w') as file:
        json_string = json.dumps(total_dur_list, default=lambda o: o.__dict__, indent=4)
        file.write(json_string)

    filepath = "/users/chbandi/traces/cpc/results/criticalPath.json"
    with open(filepath, 'w') as file:
        json_string = json.dumps(criticalPath, default=lambda o: o.__dict__, indent=4)
        file.write(json_string)

    filepath = "/users/chbandi/traces/cpc/results/traceIDList.json"
    with open(filepath, 'w') as file:
        json_string = json.dumps(traceIDList, default=lambda o: o.__dict__, indent=4)
        file.write(json_string)

    filepath = "/users/chbandi/traces/cpc/results/anamolySpanDist.json"
    with open(filepath, 'w') as file:
        json_string = json.dumps(anamoly_span_dist, default=lambda o: o.__dict__, indent=4)
        file.write(json_string)

    filepath = "/users/chbandi/traces/cpc/results/normalSpanDist.json"
    with open(filepath, 'w') as file:
        json_string = json.dumps(normal_span_dist, default=lambda o: o.__dict__, indent=4)
        file.write(json_string)
...
def critical_path_analysis(json_data, operations, root):
    ...
    if "traceIDs" not in traceIDList:
        traceIDList["traceIDs"] = []
    traceIDList["traceIDs"].append(traceID)

    if (total_duration > 15000):
        shutil.move(cpc_dir + traceID + ".json", anamoly_dir + traceID + ".json")
    else:
        shutil.move(cpc_dir + traceID + ".json", normal_dir + traceID + ".json")
...
def update_global_stats(json_data, operations):
    for op_name, data in operations.items():
        ...
        if total_duration > 15000:
            if op_name not in anamoly_span_dist:
                anamoly_span_dist[op_name] = [[], [], [], []]
            anamoly_span_dist[op_name][0].append(data["duration"])
            anamoly_span_dist[op_name][1].append(data["self_duration"])
            anamoly_span_dist[op_name][2].append(data["cp_dur"])
            anamoly_span_dist[op_name][3].append(round(data["percent_dur"], 2))
        else:
            if op_name not in normal_span_dist:
                normal_span_dist[op_name] = [[], [], [], []]
            normal_span_dist[op_name][0].append(data["duration"])
            normal_span_dist[op_name][1].append(data["self_duration"])
            normal_span_dist[op_name][2].append(data["cp_dur"])
            normal_span_dist[op_name][3].append(round(data["percent_dur"], 2))
