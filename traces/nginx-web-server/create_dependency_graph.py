...
read_user_timeline = "/wrk2-api/user-timeline/read"
read_home_timeline = "/wrk2-api/home-timeline/read"
...
def is_nginx_webserver(opname):
    if opname in [read_user_timeline, read_home_timeline, compose_posts, follow_user, register_user]:
        return True
    return False
################################################## 
...
def retrieve_operations(json_data):
    ...
    for span in json_data["spans"]:
        ...
        if is_nginx_webserver(op["parent"]):
            root = name
